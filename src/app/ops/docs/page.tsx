    ...
    <main className="mx-auto max-w-3xl py-10 space-y-6">
            ...
            {files.map((file) => {
              ...
              return (
                <a
                  key={file}
                  href={`/ops/docs/${slug}`}
                  className="block p-4 border border-gray-200 rounded-lg hover:border-[#E8FF5A] hover:bg-orange-50 transition-colors duration-200"
                >
                  <h3 className="text-[#E8FF5A] font-medium">{title}</h3>
      ...
      <div className="border-t border-gray-200 pt-6">
        <div className="flex items-center justify-between text-sm text-gray-500">
          ...
          <Link href="/" className="text-[#E8FF5A] hover:text-[#F0FF85] transition-colors">
            ← Back to Home
          </Link>
        </div>
      </div>
