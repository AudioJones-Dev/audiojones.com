    ...
    >
      <defs>
        <linearGradient id="responseosRoute" x1="72" x2="548" y1="180" y2="180" gradientUnits="userSpaceOnUse">
          ...
          <stop offset="48%" stopColor="#E8FF5A" />
          <stop offset="100%" stopColor="#3DFFB0" />
        </linearGradient>
      </defs>
      <rect x="18" y="28" width="584" height="304" rx="28" fill="rgba(15,15,15,0.72)" stroke="#2A2A2A" />
      <path d="M108 180 H512" stroke="url(#responseosRoute)" strokeWidth="8" strokeLinecap="round" />
      <path d="M492 160 L520 180 L492 200" fill="none" stroke="#3DFFB0" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" />
      {[
        [108, "LEAD"],
        [310, "AGENT"],
        [512, "RECOVERY"],
        ...
        <g key={label}>
          <circle cx={Number(x)} cy="180" r="46" fill={index === 0 ? "rgba(77,172,255,0.14)" : index === 1 ? "rgba(232,255,90,0.10)" : "rgba(61,255,176,0.14)"} stroke={index === 0 ? "#4DACFF" : index === 1 ? "#E8FF5A" : "#3DFFB0"} strokeOpacity=".72" />
          <circle cx={Number(x)} cy="180" r="10" fill={index === 0 ? "#4DACFF" : index === 1 ? "#E8FF5A" : "#3DFFB0"} />
          <text x={Number(x)} y="262" textAnchor="middle" fill="#E8E8E8" fontFamily="ui-monospace, monospace" fontSize="12" letterSpacing="3">
            {label}
          </text>
        </g>
      ))}
      <text x="310" y="82" textAnchor="middle" fill="#E8FF5A" fontFamily="ui-monospace, monospace" fontSize="12" letterSpacing="4">
        AGENT LAYER
      </text>
...
export default function ResponseOSWedge() {
  return (
    <section className="aj-bg-grid-dark border-t border-[var(--line-2)] py-16 sm:py-24">
