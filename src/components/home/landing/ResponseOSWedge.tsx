    ...
    >
      <defs>
        <linearGradient id="responseosRoute" x1="72" x2="548" y1="180" y2="180" gradientUnits="userSpaceOnUse">
          ...
          <stop offset="48%" stopColor="#F0FF85" />
          <stop offset="100%" stopColor="#4DACFF" />
        </linearGradient>
      </defs>
      <rect x="18" y="28" width="584" height="304" rx="28" fill="rgba(15,15,15,0.72)" stroke="rgba(102,102,102,0.32)" />
      ...
      {[
        ...
        <g key={label}>
          <circle cx={Number(x)} cy="180" r="46" fill={index === 0 ? "rgba(232,255,90,0.14)" : index === 1 ? "rgba(255,179,64,0.10)" : "rgba(77,172,255,0.14)"} stroke={index === 0 ? "#E8FF5A" : index === 1 ? "#FFB340" : "#4DACFF"} strokeOpacity=".72" />
          <circle cx={Number(x)} cy="180" r="10" fill={index === 0 ? "#E8FF5A" : index === 1 ? "#FFB340" : "#4DACFF"} />
          <text x={Number(x)} y="262" textAnchor="middle" fill="#9A9A9A" fontFamily="ui-monospace, monospace" fontSize="12" letterSpacing="3">
            {label}
          </text>
        </g>
      ))}
