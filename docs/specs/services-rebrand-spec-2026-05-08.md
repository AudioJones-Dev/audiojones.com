...
- ❌ No CTA destination changes to `signalDiagnostic` (PR #51 unified the constant; PR #53 set its value to `/founder-intelligence-system/diagnostic`. Both diagnostic and ROI CTAs must continue to flow through `ctaLinks` in `src/config/links.ts`).
...
- ❌ No changes to `/founder-intelligence-system`, `/roi-calculator`, `/apply`, `/blog`, or other existing routes.
...
| # | Candidate | Rationale |
|---|---|---|
| **A** | **"Founder Intelligence System Services"** | Mirrors the existing `/founder-intelligence-system` framework page; reinforces the AJ thesis; clean and operator-clinical. |
...
- ✅ New: "**Founder Intelligence System Buildout.** A 90-day engagement to install the operating system your business already needs but isn't running. We diagnose the bottleneck, deploy the AI agent that resolves it, and instrument the measurement loop. Most engagements ship in 60 days."
...
| Footer (page-bottom CTA section) | **"Take the Signal Diagnostic"** | `ctaLinks.signalDiagnostic` (= `/founder-intelligence-system/diagnostic` — see §17.1 of `docs/codex/roi-calculator-v1-brief.md` for canonical-CTA policy) | `<ButtonLink variant="glow">` | Page-bottom recovery CTA for users who've read the page but haven't yet engaged. Same canonical destination used by ROI Calculator + Founder Intelligence System + blog. |
...
- Grid (`grid gap-6 sm:grid-cols-2 lg:grid-cols-3`) of bucket cards. Each bucket card:
  ...
  - Inside the card:
    ...
    - Optional inline link `<Link>...</Link>` (text-only, no orange) → relevant section of `/founder-intelligence-system` if the bucket maps cleanly
...
- Trailing inline text link → `/founder-intelligence-system` (text-only, e.g., "Read the full Founder Intelligence System framework →")
...
`/services` is a live indexed route. Changing the H1 + meta description will cause a Google re-crawl and possible temporary ranking fluctuation. **Mitigation:** the new title (§3 candidates A/B/C) is more keyword-aligned with operator/founder/founder-intelligence-system search intent, so the long-term effect is expected positive. No 301 redirects needed (route unchanged).
...
The `/founder-intelligence-system`, `/insights`, `/apply`, `/roi-calculator` internal links must continue to resolve correctly. **Mitigation:** Codex implementation PR includes a smoke-check that all internal hrefs resolve to existing routes (no 404s in the rendered HTML).
...
- [ ] **A: "Founder Intelligence System Services"** — clinical, mirrors the existing framework page
...
> **Decided (2026-05-08): Option A — "Founder Intelligence System Services"**. Matches the site brand thesis, leverages the existing `/founder-intelligence-system` route, and aligns with the established framework section vocabulary.
...
> **Decided (2026-05-08): Option a — tools-vs-systems framing.** Anchors operator language ("We don't sell tools, we build systems") and aligns directly with the brand thesis. Carries the Q1 "Founder Intelligence System Services" headline cleanly into the subhead.
...
- **No nav restructure.** PR #50 owns `src/config/nav.ts` + Header/Footer single-source. Do not touch.
- **No CTA destination changes to `signalDiagnostic`.** PR #51 unified the constant; PR #53 set its value to `/founder-intelligence-system/diagnostic`. Both must continue to flow through `ctaLinks` in `src/config/links.ts`. No hardcoded URLs to either `/founder-intelligence-system/diagnostic` or `https://diagnostic.audiojones.com`.
...
- `docs/codex/roi-calculator-v1-brief.md §17.1` — canonical-CTA policy (locked: `/founder-intelligence-system/diagnostic` is canonical via `ctaLinks.signalDiagnostic` in v1).
